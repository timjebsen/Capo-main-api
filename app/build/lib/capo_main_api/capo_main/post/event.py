...
class event:
    ...
    def post_event(self, cursor, request, gig_info):
        ...
        try:
            ...
            # Check for blacklisted titles: 'VIP ENTRY', etc...
            if blacklisted_title(gig_info['artist_id']):
                raise BlockedEventTitle('Blocked title against: {}'.format(gig_info['artist_id']))
        ...
        except BlockedEventTitle as e:
            n.print_note(None, 2, "post_event", e)
            res = {
                "status": "FAIL",
                "message": "Failed to creat new event",
                "detailed": n.print_note(None, 2, "post_event", "Blocked event title"),
            }
            return res
    ...
    @connector_for_class_post_method
    def activate(self, cursor, data):
        try:
            res = {}
            event_id = data['event_id']
            
            # Check for conflict/duplicate before activating
            event_data = event_h_funcs.event_details(None, cursor, event_id)
            is_conflicting = event_h_funcs.check_event_conflict(None, cursor, event_data['time'], event_data['venue_id'], event_data['date'])

            if( not is_conflicting ):
                if (event_h_funcs.activate(None, cursor, event_id)):
                    res['status'] = 'OK'
                    res['message'] = 'Successfully activated event'

                else:
                    res['status'] = 'OK'
                    res['message'] = 'Failed to activate event. Event already activated?'
            else:
                res['status'] = 'OK'
                res['message'] = 'Failed to activate event. Disable conflicting event first before activating'
            return res
            ...
            res = {
                ...
                "detailed": n.print_note(None, 2, "activate", "Unexpected error in event activate. See logs for details"),
            }
            return res
    
    @connector_for_class_post_method
    def approve(self, cursor, data):
        try:
            res = {}
            event_id = data['event_id']
            
            # Check for conflict/duplicate before approving
            event_data = event_h_funcs.event_details(None, cursor, event_id)
            is_conflicting = event_h_funcs.check_event_conflict(None, cursor, event_data['time'], event_data['venue_id'], event_data['date'])

            if( not is_conflicting ):
                if (event_h_funcs.approve(None, cursor, event_id)):
                    res['status'] = 'OK'
                    res['message'] = 'Successfully approved event'

                else:
                    res['status'] = 'OK'
                    res['message'] = 'Failed to approve event. Event already approved?'
            else:
                res['status'] = 'OK'
                res['message'] = 'Failed to approve event. Disable conflicting event first before approving'
            ...
            res = {
                ...
                "detailed": n.print_note(None, 2, "approve", "Unexpected error in event approve. See logs for details"),
            }
            return res
...
def blacklisted_title(title):
    ...
    for _str in blacklist_str:
        if _str.lower() in title.lower():
            return True
