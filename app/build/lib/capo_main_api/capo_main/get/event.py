    ...
    def info(self, cursor, request):
        ...
        try:
            if len(myresult) != 0:
                ...
                event_info = {
                    "event_info": event_h_funcs.event_details(None, cursor, event_id),
...
class events_list:
    ...
    def day(self, cursor, request):
        ...
        try:
            # TODO verify values
            date_start = request.query['sdate']
            date_end = request.query['edate']
    ...
    def all(self, cursor, request):
        # Returns all events in the daatabase after current day. Used for admin
        stime = time.time()
