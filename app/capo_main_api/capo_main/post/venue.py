    ...
    try:
        ...
        try:
            ...
            if 'facebook.com' in website:
                fb_link = venue_info['socials']['website']
                website = None
            else:
                link = re.search(r"https?:\/\/[^\/]*", website)
                website = link.group(0)
        except KeyError:
            website = None
        except TypeError:
            website = None
        ...
        if not check_types(venue_info['types']):
            print(venue_info['name'])
            print(venue_info['place_id'])
            print(venue_info['types'])
            res = {}
            res['status'] = 'OK'
            res['note'] = n.print_note(
                None, 0, "post_venue_ind",
                "Venue type not accepted".format(venue_info["name"]))
            return res
        ...
        if not db_funcs.check_exist(None, cursor, table, column, places_id):
            ...
            sql = "INSERT INTO `venues` (source_id, name, address, suburb, hours_weekday, hours_raw, rating, google_place_id, uuid, has_img, description, website, facebook_link, coordinate) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, UUID(), %s, %s, %s, %s, {})".format(venue_model['coordinate'])
...
def get_places_id(request):
    try:
        ...
        elif (place_res['status'] == 'REQUEST_DENIED'):
            res['status'] = place_res['status']
            res['message'] = n.print_note(None, 2, "get_places_id", "Places API request denied. Check account.")
            return res
...
def check_types(venue_types):
    ...
    if 'bar' in venue_types or 'night_club' in venue_types:
        return True
        
    for blacklisted_type in blacklist_types:
        for venue_type in venue_types:
            if venue_type == blacklisted_type:
                # Has a blacklisted type.
                # Check if that is an outlier

                # Ignore if lodging is included with bar or nightclub types
                if 'bar' in venue_types and venue_type == 'lodging' or 'night_club' in venue_types and venue_type == 'lodging':
                    print('Venue types exception ocurred')
                    return True
                if 'bar' in venue_types and venue_type == 'tourist_attraction' or 'night_club' in venue_types and venue_type == 'tourist_attraction':
                    print('Venue types exception ocurred')
                    return True
                else:
                    return False
