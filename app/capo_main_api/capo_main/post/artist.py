...
class post_artist:
    ...
    @connector_for_class_post_method
    def post_temp_artist(self, cursor, data_json):
        ...
        # data_json  = {
        #     "event_name": str,
        #     "source_name": int # For future use
        # }
        # 
        res = {}
        try:

            source_id = db_funcs.get_source_id(None, cursor, data_json['source_name'], data_json['source_name'])
            ...
            val = (data_json['event_name'], source_id, datetime.now(), region_id)
            ...
            res['status'] = 'OK'
            res['note'] = n.print_note(None, 0, "post_temp_artist", "Successfully created a temp artist: " + data_json['event_name'] )
            res['artist_id'] = uuid
            return res
    ...
    def post_artist_ind(self, cursor, artist_info):
        ...
        try:
            ...
            if (check_exist_value != ""):
                ...
                for genre_raw in artist_info["genres"]:
                    sql = "SELECT `genre_id` FROM `genres` WHERE `genre_raw` = %s"
                    val = (genre_raw,)
                    cursor.execute(sql, val)
                    myresult = cursor.fetchall()
                    ...
                    # Add new genre if not exists
                    if not myresult:
                        sql = "INSERT INTO `genres` (genre, uuid, genre_raw) VALUES (%s, UUID(), %s)"
                        genre = string.capwords(genre_raw.replace("_", " "))
                        val = (genre,genre_raw)
                        cursor.execute(sql, val)
            ...
            res = {
                'status': 'FAIL',
                'content':{
                    'note': n.print_note(None, 2, "post_artist_ind", "Incomplete data for artist: {}. Error at: {}".format(artist_info["name"], error))
                }
            }
