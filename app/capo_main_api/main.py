...
from .capo_main import helper_funcs as h_funcs
from .capo_main.get import *
from .capo_main.post import *
from .capo_main.helper_funcs_ep import *
from .capo_main.sql.config import Config
from .capo_main.logger import Logger
# -----------------------------------------------------------------
...
# TODO standardise response objects
# -----------------------------------------------------------------
...
async def activate_event(request):
    h_funcs.h_funcs.print_req_info(None, request)
    res = {}
    try:
        data_json = await request.json()

        res = post_event.activate(None, data_json)
    except:
        traceback.print_exc()
...
async def approve_event(request):
    h_funcs.h_funcs.print_req_info(None, request)
    res = {}
    try:
        data_json = await request.json()

        res = post_event.approve(None, data_json)
    except:
        traceback.print_exc()
...
app.add_routes([
    # * Read
    web.get('/v0.1/events/today', todays_gigs),  # ! Deprecated
...
# CORS is required for admin tool functionality
...
resource4 = cors.add(app.router.add_resource("/event/activate"))
route4 = cors.add(
    resource4.add_route("POST", activate_event), {
        "*": aiohttp_cors.ResourceOptions(
            allow_credentials=True,
            expose_headers="*",
            allow_headers="*",
...
resource5 = cors.add(app.router.add_resource("/event/approve"))
route5 = cors.add(
    resource5.add_route("POST", approve_event), {
        "*": aiohttp_cors.ResourceOptions(
            allow_credentials=True,
            expose_headers="*",
            allow_headers="*",
...
resource6 = cors.add(app.router.add_resource("/search/region"))
route6 = cors.add(
    resource6.add_route("GET", search_reg), {
        "*": aiohttp_cors.ResourceOptions(
            allow_credentials=True,
            expose_headers="*",
            allow_headers="*",
...
def dev():
    ...
    if not len(args) == 2:
        raise Exception(
            'Incorrect number arguments given. Expects "local" or "prod"')

    # Set environment vars
    Config.set_env(args[1])

    # Set up logging to output to stdout and write to log file
    sys.stdout = Logger()
